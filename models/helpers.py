...
def draw_board():
    for i in range(32):
        ...
        row = i // 4
        if row % 2 == 0:
            pygame.draw.rect(screen, 'light pink', [
...
def draw_pieces():
    for i in range(len(settings.white_pieces)):
        ...
        if settings.white_pieces[i] == 'pawn':
            ...
            screen.blit(images.white_images[index], (
                settings.white_locations[i][0] * 100 + 1, settings.white_locations[i][1] * 100 + 10))
        if settings.turn_step < 2:
            if settings.selection == i:
                pygame.draw.rect(screen, 'red', [settings.white_locations[i][0] * 100 + 1, settings.white_locations[i][1] * 100 + 1,
                                                 100, 100], 2)
    for i in range(len(settings.black_pieces)):
        ...
        if settings.black_pieces[i] == 'pawn':
            ...
            screen.blit(images.black_images[index], (
                settings.black_locations[i][0] * 100 + 10, settings.black_locations[i][1] * 100 + 10))
        if settings.turn_step >= 2:
            if settings.selection == i:
                pygame.draw.rect(screen, 'blue', [settings.black_locations[i][0] * 100 + 1, settings.black_locations[i][1] * 100 + 1,
                                                  100, 100], 2)
